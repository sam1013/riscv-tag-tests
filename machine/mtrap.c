...
uintptr_t fault_continue = 0;
volatile fault_t expected_fault;

void test_fault(uintptr_t* regs, uintptr_t mcause, uintptr_t mepc)
{
  uintptr_t addr = read_csr(mtval);
  printm("Test fault %d at %p, accessing %p?\n", mcause, mepc, addr);

  if (expected_fault.cause != mcause) {
    die("Unexpected trap [%d] @ %p. Expected [%d]", mcause, mepc, expected_fault.cause);
  }
  if (expected_fault.epc != mepc) {
    die("Unexpected trap [%d] @ %p. Expected @ %p", mcause, mepc, expected_fault.epc);
  }
  if (expected_fault.resume_pc) {
    clear_csr(ststatus, TSTATUS_UI);
    printm("Resuming at %p\n", expected_fault.resume_pc);
    write_csr(mepc, expected_fault.resume_pc);
  }
}
...
static uintptr_t mcall_usertest(uint64_t arg)
{
  printm("MCALL_USERTEST %zu\n", arg);
  // TODO check: do we have a growing stack due to recursion ?
  m_usertest_main(arg);
